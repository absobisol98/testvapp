    ...
    <div class="relative h-44 overflow-hidden bg-gray-200 flex-shrink-0">
        ...
        @if($timeBadge)
            <div class="absolute top-2.5 left-2.5">
                <span class="inline-flex items-center bg-white/90 backdrop-blur-sm text-gray-800 text-[11px] font-semibold px-2.5 py-1 rounded-full shadow-sm leading-none">{{ $timeBadge }}</span>
            </div>
        @endif
    ...
    <div class="px-4 pb-4 flex items-center gap-2">
        ...
        @if($canEdit || $canDelete || $canSetFeatured)
            <div x-data="{ open: false }" style="position:relative; flex-shrink:0;">
                <button @click.stop="open = !open" type="button" class="w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" style="position:absolute; bottom:calc(100% + 6px); right:0; background:#fff; border:1px solid #e5e7eb; border-radius:10px; box-shadow:0 8px 24px rgba(0,0,0,.13); min-width:180px; z-index:9999;" @click.stop>
                    @if($canEdit)
                        <a href="{{ route('filament.admin.resources.events.edit', ['record' => $record->id]) }}" class="flex items-center gap-2.5 px-4 py-3 text-[13px] font-semibold text-[#f26522] hover:bg-gray-50 transition-colors w-full" style="display:flex; text-decoration:none; border-bottom:1px solid #f3f4f6;"><svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zm17.71-10.25a1 1 0 000-1.41l-2.34-2.34a1 1 0 00-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>Edit</a>
                    @endif
                    @if($canSetFeatured)
                        <button type="button" onclick="vappToggleFeatured('{{ $record->id }}', {{ $record->is_featured ? 'true' : 'false' }})" class="flex items-center gap-2.5 px-4 py-3 text-[13px] font-semibold text-[#d97706] w-full hover:bg-gray-50 transition-colors bg-none border-none cursor-pointer" style="display:flex; border-bottom:1px solid #f3f4f6;"><svg width="16" height="16" fill="{{ $record->is_featured ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>{{ $record->is_featured ? 'Remove Featured' : 'Make it Featured' }}</button>
                    @endif
                    @if($canDelete)
                        <button type="button" onclick="vappDelete('{{ $record->id }}')" class="flex items-center gap-2.5 px-4 py-3 text-[13px] font-semibold text-[#dc2626] w-full hover:bg-red-50 transition-colors bg-none border-none cursor-pointer" style="display:flex;"><svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M16 9v10H8V9h8m-1.5-6h-5l-1 1H5v2h14V4h-3.5l-1-1zM18 7H6v12a2 2 0 002 2h8a2 2 0 002-2V7z"/></svg>Delete</button>
                    @endif
                </div>
            </div>
        @endif
...
if (!window._vappCardHelpersLoaded) {
    ...
    window.vappDelete = function(id) {
        ...
        fetch('/admin/events/' + id, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } })
            .then(r => { if (r.ok || r.status === 204) { document.getElementById('card-' + id)?.closest('.fi-ta-col')?.remove(); if (window.Livewire) Livewire.dispatch('refresh'); } else { alert('Delete failed — check your permissions.'); } })
    ...
    window.vappToggleFeatured = function(id, isFeatured) {
        ...
        fetch('/admin/events/' + id + '/toggle-featured', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } })
            .then(r => { if (r.ok) { if (window.Livewire) Livewire.dispatch('refresh'); } else { alert('Failed — check your permissions.'); } })
            .catch(() => alert('Network error. Please try again.'));
    };
}
</script>
